...
def generate_dialogue(args, tts_id: str, user_age: int, user_awareness: str, conversation_type: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Tạo agent với base_url optional
        left_agent_kwargs = {
            "model": args.model,
        ...
        if args.base_url:
            left_agent_kwargs["base_url"] = args.base_url
        left_agent = LeftAgent(**left_agent_kwargs)        # Random chọn nghề nghiệp
        ...
        if args.base_url:
            right_agent_kwargs["base_url"] = args.base_url
        right_agent = RightAgent(**right_agent_kwargs)
        ...
        if args.base_url:
            manager_agent_kwargs["base_url"] = args.base_url
        manager_agent = ManagerAgent(**manager_agent_kwargs)
...
def main():
    ...
    api_info = args.base_url or "Gemini (built-in)"
    logger.info(f"Bắt đầu sinh {args.count} hội thoại bình thường, model: {args.model}, API: {api_info}")
    
    # Cấu hình API key và URL
    config.OPENAI_API_KEY = args.api_key
    config.OPENAI_BASE_URL = args.base_url or ""
